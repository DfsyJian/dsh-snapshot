 ...
 * ui-plugin-config card-form pattern in a self-contained slice.
 */
...
export interface CardShell {
  /** False only while the namespace is still loading; the card renders nothing. */
  available: boolean
  /**
...
export class CardForm<T> {
  ...
  field(field: string): FieldState {
    ...
    if (staged === undefined) {
      return { text: this.displayed(field), overridden: this.stored(field), invalid: false }
    }
  ...
  private plan(): PlannedWrite[] {
    ...
    for (const [field, staged] of this.staged) {
      ...
      if (staged.text === this.displayed(field)) continue
  ...
  /**
   * The draft text a field shows with no staged edit: the effective section
   * value, falling back to the composition-layer default when the user layer
   * has not touched the field, so a control always renders a concrete value
   * instead of an empty inherit slot.
   */
  private displayed(field: string): string {
    const spec = this.specOf(field)
    const raw = spec.format(this.sectionValue(field))
    return raw === '' ? spec.format(this.baseValue(field)) : raw
  }
